...
void CI_putCmd(uint8_t code, const uint8_t *data, int sz) {
	...
	USART_DBG_hexDump((uint8_t*)USART2_TxBuffer, USART2_TxBufferSz); USART_DBG_puts(" snd\n");
...
void USART2_IT_RxReady_exec(uint8_t c) {
	...
	if(USART2_RxBufferPtr > 3) {
		...
		if(USART2_RxBufferPtr > crcOfs) {
			USART_DBG_hexDump((uint8_t*)USART2_RxBuffer, USART2_RxBufferPtr); USART_DBG_puts(" rcv\n");
